...
//echo  $_SESSION["email"];
 ...
 $current_date = date('Y-m-d');
    ...
    if(isset($_POST["btn_submit"]))
                            {
                              extract($_POST);
                              $sql="INSERT INTO tbl_group (name, description)
                                values($assign_name,$description)";
                              $query=pg_query($sql);
                            $last_id =  pg_insert_id($conn);
                            $id=$last_id;
                            ...
                            for($i=0;$i<$a;$i++){
                              ...
                              $sql="INSERT INTO tbl_permission_role (permission_id, role_id)
                                values('$checkItem[$i]','$id')";
                              $query=pg_query($sql);
